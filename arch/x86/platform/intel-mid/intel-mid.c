...
#define LCD_CMDLINE_SIZE 26
#define LCD_PARAM_NAME "androidboot.lcdsize="

void update_lcdsize_cmdline(int w, int h)
{
	char *lcd_param;
	char lcd_cmdline[LCD_CMDLINE_SIZE+1];

	if( w<=0 || h <=0 || w >=10000 || h >=10000)
	{
		printk("Only accept 4 digits number at most\n");
		return;
	}
	/* parameter format : wwwwxhhhh */
	snprintf(lcd_cmdline, sizeof(lcd_cmdline), "%04dx%04d", w, h);

	/* is there a lcd param ? */
	lcd_param = strstr(saved_command_line, LCD_PARAM_NAME);
	if (lcd_param) {
		/* is the param set to default value ? */
			lcd_param += strlen(LCD_PARAM_NAME);
			if (strlen(lcd_param) > strlen(lcd_cmdline))
				memcpy(lcd_param, lcd_cmdline,
							strlen(lcd_cmdline));
			else
				pr_err("Not enough free space for LCDSIZE in command line.\n");
	} else
		pr_err("LCDSIZE not found in kernel command line.\n");
}
EXPORT_SYMBOL(update_lcdsize_cmdline);
