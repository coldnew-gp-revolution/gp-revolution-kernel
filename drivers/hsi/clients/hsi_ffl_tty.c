...
static void ffl_do_tty_forward(struct work_struct *work)
{
	...
	if (unlikely(atomic_read(&ffl_drv.drv_remove_ongoing))) {
...
static void ffl_tty_port_shutdown(struct tty_port *port)
{
	...
	struct ffl_xfer_ctx	*rx_ctx;
	unsigned long		flags;
	pr_err(DRVNAME "ffl_tty_port_shutdown\n");
	if (unlikely(atomic_read(&ffl_drv.drv_remove_ongoing))) {
		pr_err(DRVNAME ": %s: Driver is currently removed by the system",
				__func__);
		return;
...
static int ffl_driver_probe(struct device *dev)
{
  ...
  atomic_set(&ffl_drv.drv_remove_ongoing, 0);
...
static int ffl_driver_shutdown(struct device *dev)
{
	struct hsi_client	*client = to_hsi_client(dev);

	pr_err(DRVNAME "ffl_driver_shutdown\n");
/*	if (unlikely(atomic_read(&ffl_drv.drv_remove_ongoing))) {
		pr_err(DRVNAME ": %s: Driver is currently removed by the system",
				__func__);
		return -ENODEV;
	...
	atomic_set(&ffl_drv.drv_remove_ongoing, 1);

	if (hsi_port_claimed(client)) {
		pr_warn(DRVNAME ":Releasing the HSI controller\n");
		hsi_release_port(client);
	}
