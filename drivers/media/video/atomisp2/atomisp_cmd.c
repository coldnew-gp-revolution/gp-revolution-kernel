...
/**************************************************************
*In falcon, the flash is drived by GPIO, which is not compatible
*with Intel's standard flash driver, comment below code lines
*for it to work
**************************************************************/
#if 0
				 ...
				 struct atomisp_sensor_mode_data *config)
{
	...
	mipi_info->data.output_width -= pad_w;
	mipi_info->data.output_height -= pad_h;
	mipi_info->data.binning_factor_x = (1 << mipi_info->data.binning_factor_x);
	mipi_info->data.binning_factor_y = (1 << mipi_info->data.binning_factor_y);
	mipi_info->data.crop_horizontal_end -= ((mipi_info->data.binning_factor_x) * pad_w - 1);
	mipi_info->data.crop_vertical_end -= ((mipi_info->data.binning_factor_y) * pad_h - 1);
			  ...
			  unsigned int dvs_env_w, unsigned int dvs_env_h)
{
	...
	printk( "sensor width: %d, height: %d\n",
