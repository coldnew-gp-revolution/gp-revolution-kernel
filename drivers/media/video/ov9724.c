				...
				struct camera_mipi_info *info)
{
	...
	buf->binning_factor_x = ov9724_res[dev->fmt_idx].bin_factor_x;
	buf->binning_factor_y = ov9724_res[dev->fmt_idx].bin_factor_y;
...
static int ov9724_t_hflip(struct v4l2_subdev *sd, int value)
{
	...
	if (ret){
   		/* disable group hold */
		ov9724_write_reg_array(c, ov9724_param_update);
		return ret;
	}
...
ov9724_s_config(struct v4l2_subdev *sd, int irq, void *platform_data)
{
	...
	if (dev->platform_data->platform_init) {
		...
		if (ret) {
			mutex_unlock(&dev->input_lock);
			dev_err(&client->dev, "ov9724 platform init err\n");
			return ret;
		}
	}
...
static int ov9724_s_stream(struct v4l2_subdev *sd, int enable)
{
    ...
    u16 reg = 0x4837;
    u16 value = 0x0;
	/*ret = ov9724_read_reg(client, OV9724_8BIT, reg, &value);
    printk("%s() in, reg = %x, value = %x\n", __func__, reg, value);*/
	...
	if (enable) {
		...
		// Ever msleep between stream modes.
		msleep(50);
        ov9724_write_reg(client, OV9724_8BIT, reg, value);

		// Little delay to ensure that the previus writereg is applied.
		msleep(50);
...
ov9724_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *param)
{
	...
	switch (dev->run_mode) {
	case CI_MODE_VIDEO:
        //printk("%s, case CI_MODE_VIDEO:\n", __func__);
        dev->stream_mode = 2;
		ov9724_res = ov9724_res_video;
		N_RES = N_RES_VIDEO;
		break;
	case CI_MODE_STILL_CAPTURE:
        //printk("%s, CI_MODE_STILL_CAPTURE:\n", __func__);
        dev->stream_mode = 1;
...
module_i2c_driver(ov9724_driver);
