...
static int N_RES;
static int IMX_EEPROM_DATA_SIZE;
static int s_config_done;

static u8* otp_data = NULL;
...

{
	...
	if (coarse_itg > dev->lines_per_frame - 4) {
		frame_length = coarse_itg + 4;
	} else {
		frame_length = dev->lines_per_frame;
	}
	//printk("coarse_intg:%d frame_length:%d \n", coarse_itg, frame_length);

	// Only for testing vcm.
#ifdef TEST_VCM
	++g_vcm_test;

//	if (g_vcm_test%4 == 0) {
//		afValue = 20;
//	} else if (g_vcm_test%4 == 1) {
//		c
//	} else if (g_vcm_test%4 == 2) {
//		afValue = 160;
//	} else {
//		afValue = 200;
//	}
	...
	if (0 != vm149c_t_focus_vcm(sd, afValue)) {
		printk("vm149c_t_focus_vcm() calling failed \n");
	}
#endif
	...
	ret = imx_write_reg(client, IMX_16BIT, IMX_FRAME_LENGTH_LINES, frame_length);
	if (ret)
		goto out;
				   ...
				   struct v4l2_private_int_data *priv)
{
	...
	if (otp_data[0] != 0x01) {
		priv->size = 0;
		printk("Invalid NVM data\n");
		return 0;
	}
	/* Correct read_size value only if bigger than maximum */
	if (read_size > IMX_EEPROM_DATA_SIZE)
		read_size = IMX_EEPROM_DATA_SIZE;
	ret = copy_to_user(to, otp_data, read_size);
	if (ret) {
...
out:
	/* Return correct size */
	priv->size = IMX_EEPROM_DATA_SIZE;
				...
				const struct imx_reg *reglist)
{
	...
	if (dev->sensor_id == IMX111_ID) {
		ret = imx_read_reg(client, 1, IMX_PLL_MULTIPLIER_IMX111, data);
		if (ret)
			return ret;
	} else {
	...
	if (ret)
		return ret;
	buf->crop_horizontal_start = data[0] - IMX111_FULL_SIZE_H_START;

	ret = imx_read_reg(client, 2, IMX_VERTICAL_START_H, data);
	if (ret)
		return ret;
	buf->crop_vertical_start = data[0] - IMX111_FULL_SIZE_V_START;

	ret = imx_read_reg(client, 2, IMX_HORIZONTAL_END_H, data);
	if (ret)
		return ret;
	buf->crop_horizontal_end = data[0] - IMX111_FULL_SIZE_H_START;

	ret = imx_read_reg(client, 2, IMX_VERTICAL_END_H, data);
	if (ret)
		return ret;
	buf->crop_vertical_end = data[0] - IMX111_FULL_SIZE_V_START;
...
static int falcon_t_flash_gpio(struct v4l2_subdev *sd, s32 value)
{
	struct imx_device *dev = to_imx_sensor(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret;
	if(value) {
		ret = dev->platform_data->flash_ctrl(sd, 1);
		if (ret)
			dev_err(&client->dev, "flash failed\n");
	} else {
		ret = dev->platform_data->flash_ctrl(sd, 0);
		if (ret)
			dev_err(&client->dev, "flash failed\n");
	}
	return 0;
...
static int falcon_t_torch_gpio(struct v4l2_subdev *sd, s32 value)
{
	struct imx_device *dev = to_imx_sensor(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret;
	if(value) {
		ret = dev->platform_data->torch_ctrl(sd, 1);
		if (ret)
			dev_err(&client->dev, "torch failed\n");
	} else {
		ret = dev->platform_data->torch_ctrl(sd, 0);
		if (ret)
			dev_err(&client->dev, "torch failed\n");
	}
	return 0;
...
struct imx_control imx_controls[] = {
	...
	},
	{
		...
		{
		.qc = {
			.id = V4L2_CID_FLASH_GPIO_MODE,
			.type = V4L2_CTRL_TYPE_INTEGER,
			.name = "flash gpio ctrl",
			.minimum = 0,
			.maximum = 1,
			.step = 1,
			.default_value = 0,
		...
		.tweak = falcon_t_flash_gpio,
	},
	{
		.qc = {
			.id = V4L2_CID_TORCH_GPIO_MODE,
			.type = V4L2_CTRL_TYPE_INTEGER,
			.name = "torch gpio ctrl",
			.minimum = 0,
			.maximum = 1,
			.step = 1,
			.default_value = 0,
			      ...
			      struct v4l2_mbus_framefmt *fmt)
{
	...
	/* FIXME: workround for MERR Pre-alpha due to ISP perf - end */
        ...
        printk("%s %d index:%d width:%d height:%d\n", __func__, __LINE__,
				dev->fmt_idx,
				imx_res[dev->fmt_idx].width,
				imx_res[dev->fmt_idx].height);
	ret = imx_write_reg_array(client, imx_param_update);
	if (ret) {
		mutex_unlock(&dev->input_lock);
		return -EINVAL;
	}
	ret = imx_write_reg(client, IMX_8BIT, 0x3040, 0x00);
	ret = imx_write_reg(client, IMX_8BIT, 0x3041, 0x00);
...
static struct device_attribute imx111_attrs = __ATTR(imx111, S_IRUGO,
						     NULL, NULL);
static int imx_detect(struct i2c_client *client, u16 *id, u8 *revision)
{
	...
	ret = device_create_file(&(client->dev), &imx111_attrs);
	if (ret)
		v4l2_info(client, "Create file imx111 fail\n");
found:
...
static int imx_eeprom_read(struct v4l2_subdev *sd, u8 *buf, int regAddr, unsigned int dataSize)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	unsigned int size;
	unsigned int eeprom_i2c_addr = IMX_EEPROM_I2C_ADDR;
	static const unsigned int max_read_size = IMX_MAX_WRITE_BUF_SIZE;
	struct i2c_msg msg[2];
	int addr;

	//int i = 0;
	int len = 0;
	size = regAddr + dataSize;
	memset(msg, 0, sizeof(msg));
// For LITEON module
	for (addr = regAddr; addr < size; addr += max_read_size) {
		unsigned char addr_buf;
		int r;
		//eeprom_i2c_addr |= (addr >> 8) & 1;
		eeprom_i2c_addr = IMX_EEPROM_I2C_ADDR | ((addr >> 8) & 3);
		addr_buf = addr & 0xFF;
		...
		if (r != ARRAY_SIZE(msg))
			dev_err(&client->dev,
				"failed to read eeprom data at 0x%02x\n", addr);

		//printk("imx_eeprom_read(), i2c address = 0x%02x, reg address = 0x%02x\n", eeprom_i2c_addr, addr);
		//for (i = 0; i < len; i++) {
			//printk("imx_eeprom_read(), vaule = 0x%02x\n", *(buf + addr + i));
		//}
	}
	return 0;
}

static int imx_eeprom_read_for_liteon_module(struct v4l2_subdev *sd, u8 *buf) {
	int regAddr;
	int dataSize;
	int ret = 0;
	u8 *buff = buf;
	...
	regAddr = 0x7;
	dataSize = 16;
	ret = imx_eeprom_read(sd, buff, regAddr, dataSize);
	if(*(buf + 20) == 0x01) {
		IMX_EEPROM_DATA_SIZE = 281;
		dataSize = 16;
		buff -= regAddr;
		ret = imx_eeprom_read(sd, buff, regAddr, dataSize);
		buff += regAddr;
			    ...
			    int irq, void *pdata)
{
	...
	if (!otp_data)
		otp_data = kmalloc(IMX_EEPROM_SIZE, GFP_KERNEL);

	if (!otp_data) {
		v4l2_err(client, "imx power-down err.\n");
	} else {
		otp_data[0] = 0xff;
		//imx_eeprom_read(sd, dev->otp_data);
		imx_eeprom_read_for_liteon_module(sd, otp_data);
	}
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (dev->sensor_id == IMX111_ID)
		snprintf(dev->sd.name, sizeof(dev->sd.name), "%s %d-%04x",
		"imx111", i2c_adapter_id(client->adapter),
		client->addr);
