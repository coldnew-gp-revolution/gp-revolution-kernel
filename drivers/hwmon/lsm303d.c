...
static int lsm303d_suspend(struct device *dev)
{
	...
	struct lsm303d_status *stat = i2c_get_clientdata(i2c_client);

	printk(KERN_INFO "lsm303d : Suspend\n");

	if(atomic_read(&stat->enabled_acc)) {
		lsm303d_acc_disable(stat);
		stat->need_resume_acc = 1;
	}
	if(atomic_read(&stat->enabled_mag)) {
		lsm303d_mag_disable(stat);
		stat->need_resume_mag = 1;
	}
	if(atomic_read(&stat->enabled_temp)) {
		lsm303d_temperature_disable(stat);
		stat->need_resume_temp = 1;
	}
...
static int lsm303d_resume(struct device *dev)
{
	...
	printk(KERN_INFO "lsm303d : Resume\n");

	if (stat->need_resume_acc == 1) {
		lsm303d_acc_enable(stat);
		stat->need_resume_acc = 0;
	}
	if (stat->need_resume_mag == 1) {
		lsm303d_mag_enable(stat);
		stat->need_resume_mag = 0;
	}
	if (stat->need_resume_temp == 1) {
		lsm303d_temperature_enable(stat);
		stat->need_resume_temp = 0;
	}
...
static void lsm303d_early_suspend(struct early_suspend *h)
{
	...
	printk(KERN_INFO "lsm303d : early_suspend\n");
	stat  = container_of(h, struct lsm303d_status, early_suspend);
	dev = &stat->client->dev;
	if (lsm303d_suspend(dev) != 0)
...
static void lsm303d_late_resume(struct early_suspend *h)
{
	struct lsm303d_status *stat;
	struct device *dev;
	printk(KERN_INFO "lsm303d : late_resume\n");
	stat = container_of(h, struct lsm303d_status, early_suspend);
	dev = &stat->client->dev;
	if (lsm303d_resume(dev) != 0)
