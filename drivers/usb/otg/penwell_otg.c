...
static int penwell_otg_set_vbus(struct usb_otg *otg, bool enabled)
{
	...
	/* Charger HW workaround for SMB347 */
	/* Wait for charger enable OTG VBUS done, then A-device drives vbus */
	if (is_clovertrail(to_pci_dev(pnw->dev))) {
		if (enabled) {
			usleep_range(5000, 8000);
			dev_info(pnw->dev, "A-device drives vbus, set vbus on!\n");
		}
	}
...
static void penwell_otg_notify_warning(int warning_code)
{
	...
	if (pnw->iotg.otg.otg->host && pnw->iotg.otg.otg->host->root_hub)
	...
	case OTG_STATE_B_IDLE:
		if (hsm->id == ID_A || hsm->id == ID_ACA_A) {
			...
			if (charger_type == CHRG_SE1) {
				...
				/* SE1: set charger type to DCP, current, notify EM */
		...
		const struct pci_device_id *id)
{
	...
	pnw->iotg.otg.otg->phy = &pnw->iotg.otg;
