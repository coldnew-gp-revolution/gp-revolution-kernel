...
extern void ft5x06_ts_filter(bool enabled);
#endif
...
static ssize_t smb347_set_charge_current_limit(struct device *device,
                         struct device_attribute *attr, const char *buf,
                         size_t count);
static DEVICE_ATTR(charge_current_limit, S_IRUGO | S_IWUSR,
                                         NULL,
                                         smb347_set_charge_current_limit);

static ssize_t smb347_set_charge_current_limit(struct device *dev,
                                struct device_attribute *attr, const char *buf,
                                size_t count)
{
	long value;
	...
	switch (value) {
	case 0:
		ret = smb347_set_usb_suspend(smb347_dev, true);
		if (ret < 0)
			dev_err(&smb347_dev->client->dev,"failed to disenable charging\n");

		dev_warn(&smb347_dev->client->dev, "ATTR disable the charging\n");
		break;
	case 1:
		ret = smb347_set_usb_suspend(smb347_dev, false);
		if (ret < 0)
			dev_err(&smb347_dev->client->dev,"failed to enable charging\n");

		dev_warn(&smb347_dev->client->dev, "ATTR enable the charging\n");
		break;
	default:
		dev_warn(&smb347_dev->client->dev, "the paraneter no support\n");
	}
	smb347_set_writable(smb347_dev, false);
...
static void smb347_event_worker(struct work_struct *work)
{
	...
	case POWER_SUPPLY_CHARGER_EVENT_RESUME:
		if (smb->cap.chrg_type != POWER_SUPPLY_TYPE_USB_HOST) {
			...
			mutex_unlock(&smb->lock);
#ifdef CONFIG_TOUCHSCREEN_FT5x06
			/*workaround for tp issue when wall charger plug*/
			ft5x06_ts_filter(true);
#endif
		}
#if 0
		/*
			...
			const struct i2c_device_id *id)
{
	...
	schedule_delayed_work(&smb->smb347_statmon_worker, 0);
        ret = device_create_file(&smb->client->dev,
