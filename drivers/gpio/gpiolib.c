...
static void gpiolib_dbg_show(struct seq_file *s, struct gpio_chip *chip)
{
	...
	for (i = 0; i < chip->ngpio; i++, gpio++, gdesc++) {
#ifndef CONFIG_INTEL_EXTENDED_SUPPORT
		...
		if( chip->arch_dbg_info )
			chip->arch_dbg_info(s, chip, gpio);
#endif
		seq_printf(s, "\n");
	}
}
...
{
	unsigned int gpio_num=0,val=0;
	char gpio_buf[4];
	char messages[8];

	if (copy_from_user(messages, buf, min(6,len)))
		return -EFAULT;

	messages[6]='\0';
	printk("input str=%s\n",messages);
	memcpy(gpio_buf, messages+1, 3);
	gpio_buf[3]='\0';
	gpio_num = (int) simple_strtoul(gpio_buf, NULL, 10);
	printk("set gpio_num=%d\n",gpio_num);

	if( gpio_num >= ARCH_NR_GPIOS )
		return -EINVAL;
	if( ('+'== messages[0] ) ) {
		gpio_direction_input(gpio_num);
	}else if (('-' == messages[0]) && '=' == messages[4] ) {
		/* format correct, */
		val = (messages[5]=='0')?0:1;
		{
			gpio_direction_output(gpio_num,val);
			printk("gpio_lib:set gpio%d to %s\n",gpio_num, val?"high":"low");
		}
	}else /* format error */
		return -EFAULT;
	return len;
