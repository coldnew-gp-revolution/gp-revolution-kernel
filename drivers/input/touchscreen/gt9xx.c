...
enum {
	TOUCH_GT911 = 0, //cfg_group1
	TOUCH_GT928,	//cfg_group2
	TOUCH_GT915,	//cfg_group3
};
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    GTP_GPIO_OUTPUT(GTP_RST_PORT, 1);   //begin select I2C slave addr
    msleep(ms);
    ...
    GTP_GPIO_OUTPUT(GTP_INT_PORT, 0);    //let guitar begin to work
    msleep(50);
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    GTP_INFO("len1=%d,len2=%d,len3=%d,send_len:%d",cfg_info_len[0],cfg_info_len[1],cfg_info_len[2],ts->gtp_cfg_len);
    ...
    rd_cfg_buf[GTP_ADDR_LENGTH] = sensor_type;
...
s32 gtp_read_version(struct i2c_client *client, u16* version)
{
    ...
    GTP_INFO("IC version buf: %s\n", buf);
    if (strstr(buf, "911")) {
        sensor_type = TOUCH_GT911;
    } else if (strstr(buf, "928")) {
        sensor_type = TOUCH_GT928;
    } else if (strstr(buf, "915")) {
	sensor_type = TOUCH_GT915;
    }
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    ts->input_dev->phys = "goodix-ts/input0";
...
ERR_HW_INIT:
	GTP_ERROR("I2c test failed and free the gpio");
	GTP_GPIO_FREE(GTP_RST_PORT);
	GTP_GPIO_FREE(GTP_INT_PORT);
