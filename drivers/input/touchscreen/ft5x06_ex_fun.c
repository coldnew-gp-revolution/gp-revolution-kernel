...
/*fw version check for fw upgrade*/
#define FTS_AUTO_UPGRADEG_CHECK_VER
...
static unsigned char CTPM_FW[] = {
	#include "touch5336.i"
	//#include "ast_Ver30_romosW32_20130816_app.i"
};
...
u8 fts_ctpm_get_i_file_ver(void)
{
	...
	printk(KERN_INFO "%s: ui_sz is 0x%x\n",__func__,ui_sz);
	if (ui_sz > 2)
...
{
	u8 uc_host_fm_ver = FT5X0X_REG_FW_VER; //0xa6
	u8 uc_tp_fm_ver;
	int i_ret;
	...
	pr_debug("[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver);

#ifdef FTS_AUTO_UPGRADEG_CHECK_VER
...
#endif
	{
		...
		if (i_ret == 0)	{
			msleep(300);
			uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			pr_info("[FTS] upgrade to new version 0x%x\n",uc_host_fm_ver);
		} else {
			  ...
			  u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		/*********Step 1:Reset  CTPM *****/
		/*write 0xaa to register 0xfc */
		DBG("[FTS] Step 3: delay_aa=%d,delay_55 = 0x%d\n",
				upgradeinfo.delay_aa, upgradeinfo.delay_55);
		...
		upgradeinfo.delay_aa = upgradeinfo.delay_aa + 10;
		...
		upgradeinfo.delay_55 = upgradeinfo.delay_55 +10;
		/*********Step 2:Enter upgrade mode *****/
	...
	const char __user *buff, unsigned long len, void *data)
{
	...
	unsigned long buflen = min(len,(unsigned long)(sizeof(writebuf) -1));
	int writelen = 0;
	int ret = 0;
