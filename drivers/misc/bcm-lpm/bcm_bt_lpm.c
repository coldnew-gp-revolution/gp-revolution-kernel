...
static int bcm43xx_bt_rfkill_set_power(void *data, bool blocked)
{
	...
	if (!blocked) {
		...
		*/
		...
		gpio_set_value(bt_lpm.gpio_reset_bt, 1);
		usleep_range(10, 50);
		gpio_set_value(bt_lpm.gpio_enable_bt, 1);
		pr_debug("%s: turn BT on\n", __func__);
	} else {
		gpio_set_value(bt_lpm.gpio_reset_bt, 0);
		usleep_range(10, 50);
...
static int bcm43xx_bluetooth_probe(struct platform_device *pdev)
{
	...
	bt_lpm.gpio_reset_bt = pdata->gpio_reset;
	...
	ret = gpio_request(bt_lpm.gpio_reset_bt, pdev->name);
	if (ret < 0) {
		pr_err("%s: Unable to request gpio %d\n", __func__,
							bt_lpm.gpio_reset_bt);
		goto err_gpio_enable_req;
	}

	ret = gpio_direction_output(bt_lpm.gpio_reset_bt, 0);
	if (ret < 0) {
		pr_err("%s: Unable to set int direction for gpio %d\n",
					__func__, bt_lpm.gpio_reset_bt);
		goto err_gpio_enable_dir;
	}
...
err_gpio_enable_dir:
	...
	gpio_free(bt_lpm.gpio_reset_bt);
err_gpio_enable_req:
	return ret;
}
