...
/* REVERTME:
 * this is a workaround for falcon ifwi RC2. Will be reverted when new ifwi is ready.
 */
...
static ssize_t apds990x_prox_ppcount_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	struct apds990x_chip *chip = dev_get_drvdata(dev);
	return sprintf(buf, "%d\n", chip->ppcount);
...
static ssize_t apds990x_prox_ppcount_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t len)
{
	struct apds990x_chip *chip =  dev_get_drvdata(dev);
	...
	if ((value > APDS_PPCOUNT_RANGE) || (value <= 0))
		return -EINVAL;
...
static DEVICE_ATTR(prox0_ppcount, S_IRUGO | S_IWUSR,
		apds990x_prox_ppcount_show,
		apds990x_prox_ppcount_store);
...
static int apds990x_setup_irq(struct apds990x_chip *chip)
{
	 ...
	 * this is a workaround for falcon ifwi RC2. Will be reverted when new ifwi is ready.
	 */
	dev_info(&client->dev, "Set flis to \"pull up, 20K\" for PIN 0x%x.\n",
								uart_0_rts);
	config_pin_flis(uart_0_rts, PULL, UP_20K);
...
static void apds990x_init_params(struct apds990x_chip *chip)
{
	...
	chip->ppcount = chip->pdata->ppcount;
