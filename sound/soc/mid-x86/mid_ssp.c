...
enum {
    MID_DAI_0_ENABLED = 0x1,
    MID_DAI_1_ENABLED = 0x2
};

int mid_dais_status = 0;
			   ...
			   struct snd_soc_dai *cpu_dai)
{
    ...
    /*set global status*/
    if (cpu_dai->driver->id == 0) {
        mid_dais_status |= MID_DAI_0_ENABLED;
    } else if (cpu_dai->driver->id == 1) {
        mid_dais_status |= MID_DAI_1_ENABLED;
    }
	...
	int clk_id, unsigned int freq, int dir)
{
	...
	case 8000:
        ...
        if (mid_dais_status == (MID_DAI_1_ENABLED | MID_DAI_0_ENABLED)) {
            /* Scrambling mode*/
